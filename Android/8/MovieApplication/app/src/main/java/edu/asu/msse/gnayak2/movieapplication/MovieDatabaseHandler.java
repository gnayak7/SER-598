...
public class MovieDatabaseHandler extends SQLiteOpenHelper {
    ...
    public void onCreate(SQLiteDatabase db) {
        ...
        String createQuery = "CREATE TABLE " + MovieDatabaseContract.MovieEntry.TABLE_NAME + "(" +
                MovieDatabaseContract.MovieEntry.MOVIE_ID + " TEXT PRIMARY KEY , " +
                MovieDatabaseContract.MovieEntry.MOVIE_TITLE + " TEXT , " +
                MovieDatabaseContract.MovieEntry.MOVIE_YEAR + " TEXT , " +
                MovieDatabaseContract.MovieEntry.MOVIE_RATED + " TEXT , " +
                MovieDatabaseContract.MovieEntry.MOVIE_RELEASED + " TEXT , " +
                MovieDatabaseContract.MovieEntry.MOVIE_RUNTIME + " TEXT , " +
                MovieDatabaseContract.MovieEntry.MOVIE_GENRE + " TEXT , " +
                MovieDatabaseContract.MovieEntry.MOVIE_ACTORS + " TEXT , " +
                MovieDatabaseContract.MovieEntry.MOVIE_PLOT + " TEXT , " +
                MovieDatabaseContract.MovieEntry.MOVIE_POSTER + " TEXT , " +
                MovieDatabaseContract.MovieEntry.MOVIE_MEDIA + " TEXT " +
                ")" + ";";
    ...
    private ContentValues getContentValues(Movie movie, boolean includeMovieId) {
        ...
        contentValues.put(MovieDatabaseContract.MovieEntry.MOVIE_MEDIA, movie.getMedia());
    ...
    private Movie getMovieFromCursor(Cursor cursor) {
        ...
        String poster = cursor.getString(cursor.getColumnIndex(MovieDatabaseContract.MovieEntry.MOVIE_POSTER));
        String media = cursor.getString(cursor.getColumnIndex(MovieDatabaseContract.MovieEntry.MOVIE_MEDIA));
        return new Movie(id, title, year, rated, released, runtime, genre, actors, plot, poster, media);
    }
    ...
    public boolean isMoviePresentInDatabase(String title) {
        SQLiteDatabase db = getWritableDatabase();

        String querySelection = "SELECT " + MovieDatabaseContract.MovieEntry.MOVIE_ID + " FROM " + MovieDatabaseContract.MovieEntry.TABLE_NAME + " WHERE " + MovieDatabaseContract.MovieEntry.MOVIE_TITLE + " = ? ";
        Cursor cursor = db.rawQuery(querySelection, new String[]{title});
        ...
        if (cursor.getCount() == 0) {
            cursor.close();
            db.close();
            Log.w(this.getClass().getSimpleName(), " Movie does not exist in database ");
            return false;
        } else {
            cursor.moveToFirst();
            String movieId = cursor.getString(cursor.getColumnIndex(MovieDatabaseContract.MovieEntry.MOVIE_ID));
            Log.w(this.getClass().getSimpleName(), " Movie exists in database with id: " + movieId);
            // Close the cursor first before closing the database. Else cursor will be invalid.
            cursor.close();
            db.close();
            return true;
    ...
    public String getMediaForMovie(String id) {
        SQLiteDatabase db = getWritableDatabase();

        String querySelection = "SELECT " + MovieDatabaseContract.MovieEntry.MOVIE_MEDIA + " FROM " + MovieDatabaseContract.MovieEntry.TABLE_NAME + " WHERE " + MovieDatabaseContract.MovieEntry.MOVIE_ID + " = ? ";
        Cursor cursor = db.rawQuery(querySelection, new String[]{id});
        ...
        if (cursor.getCount() == 0) {
            ...
            cursor.moveToFirst();
            String mediaName = cursor.getString(cursor.getColumnIndex(MovieDatabaseContract.MovieEntry.MOVIE_MEDIA));
            Log.w(this.getClass().getSimpleName(), " Movie media name: " + mediaName);
            // Close the cursor first before closing the database. Else cursor will be invalid.
            cursor.close();
            db.close();
            return mediaName;
    ...
    public void clearDatabase() {
        ...
        db.delete(MovieDatabaseContract.MovieEntry.TABLE_NAME, "1", null);
    }
