...
class MovieLibrary {
    ...
    func getMovies() -> Dictionary<String, Movie>{
        return movies
    }
    
    init (models: Array<Model>) {
        self.models = models
        self.movies = Dictionary<String, Movie>()
    }
    ...
    func isMoveCached(groupPosition: Int, childPosition: Int) -> Bool{
        let movieName: String = self.models[groupPosition].getMovieNameAt(childPosition)
        return movies[movieName] != nil
    }
    
    //-------------- special functin addee
    func getMovieName(groupPosition: Int, childPosition: Int) -> String {
        let movieName: String = self.models[groupPosition].getMovieNameAt(childPosition)
...
//        movies.updateValue(movie, forKey: movie.getTitle())
    ...
    func editMovie(groupPostion:Int, childPostion:Int, editedMovie: Movie) -> Bool {     
        let oldMovie = getMovie(groupPostion, childPosition: childPostion)
        if (oldMovie.getGenre() == editedMovie.getGenre() && oldMovie.getTitle() == editedMovie.getTitle()) {
    ...
    func addMovieToCache(mov:Movie) {
        self.movies.updateValue(mov, forKey: mov.getTitle())
    }
    
    func deleteAndAdd(groupPosition:Int, childPosition: Int, newMovie:Movie) {
        self.models[groupPosition].delete(childPosition)
        if (models[groupPosition].getCount() == 0) {
            models.removeAtIndex(groupPosition)
        }
        addOrUpdateModelAndMovies(newMovie)
    }
